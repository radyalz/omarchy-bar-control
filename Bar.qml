...
Item {
  ...
  function loadAutohideSettings(raw) {
    ...
    if (modes.indexOf(data.animationMode) !== -1)
      autohideSettings.animationMode = data.animationMode

    if (isFinite(Number(data.showSlideDuration)))
      autohideSettings.showSlideDuration = Math.max(50, Math.min(3000, Math.round(Number(data.showSlideDuration))))
    if (isFinite(Number(data.showFadeDuration)))
      autohideSettings.showFadeDuration = Math.max(50, Math.min(3000, Math.round(Number(data.showFadeDuration))))
    if (isFinite(Number(data.hideSlideDuration)))
      autohideSettings.hideSlideDuration = Math.max(50, Math.min(3000, Math.round(Number(data.hideSlideDuration))))
    if (isFinite(Number(data.hideFadeDuration)))
      autohideSettings.hideFadeDuration = Math.max(50, Math.min(3000, Math.round(Number(data.hideFadeDuration))))
    if (isFinite(Number(data.slideDistancePercent)))
      autohideSettings.slideDistancePercent = Math.max(0, Math.min(200, Math.round(Number(data.slideDistancePercent))))

    var easings = ["Quad", "Cubic", "Quart", "Quint", "Sine"]
    if (easings.indexOf(data.easing) !== -1)
      autohideSettings.easing = data.easing

    if (typeof data.customCurveEnabled === "boolean")
      autohideSettings.customCurveEnabled = data.customCurveEnabled
    if (isFinite(Number(data.curveX1)))
      autohideSettings.curveX1 = Math.max(0, Math.min(1, Number(data.curveX1)))
    if (isFinite(Number(data.curveY1)))
      autohideSettings.curveY1 = Math.max(-1, Math.min(2, Number(data.curveY1)))
    if (isFinite(Number(data.curveX2)))
      autohideSettings.curveX2 = Math.max(0, Math.min(1, Number(data.curveX2)))
    if (isFinite(Number(data.curveY2)))
      autohideSettings.curveY2 = Math.max(-1, Math.min(2, Number(data.curveY2)))

    if (typeof data.appearanceOverrideEnabled === "boolean")
      autohideSettings.appearanceOverrideEnabled = data.appearanceOverrideEnabled
    if (isFinite(Number(data.islandEdgeMargin)))
      autohideSettings.islandEdgeMargin = Math.max(0, Math.min(64, Math.round(Number(data.islandEdgeMargin))))
    if (isFinite(Number(data.islandPadding)))
    ...
    if (isFinite(Number(data.islandOpacity)))
      autohideSettings.islandOpacity = Math.max(0.05, Math.min(1, Number(data.islandOpacity)))

    if (typeof data.barSizeOverrideEnabled === "boolean")
      autohideSettings.barSizeOverrideEnabled = data.barSizeOverrideEnabled
    if (isFinite(Number(data.barThickness)))
    ...
    if (positions.indexOf(data.position) !== -1)
      autohideSettings.positionOverride = data.position

    if (typeof data.transparent === "boolean") {
      autohideSettings.transparentOverrideSet = true
  ...
  readonly property int islandEdgeMargin: customIslandAppearance
  ...
  readonly property int islandGap: customIslandAppearance
    ? root.autohideService.islandGap : Style.space(4)
  readonly property real islandOpacity: customIslandAppearance
    ? root.autohideService.islandOpacity : 1.0
  readonly property int islandStep: islandPadX * 2 + islandGap
  ...
  // Autohide is owned by the custom bar itself. Newer Omarchy versions expose
  // shell.bar to third-party services as read-only state, so a service can no
  // longer drive barHidden directly.
  Timer {
    interval: 50
    repeat: true
  ...
  Variants {
    ...
    delegate: Component {
      PanelWindow {
        ...
        WlrLayershell.namespace: "radyalz-bar-control-trigger"
        WlrLayershell.layer: WlrLayer.Overlay
        WlrLayershell.keyboardFocus: WlrKeyboardFocus.None

        HoverHandler {
          onHoveredChanged: root.setAutohideEdgeHovered(hovered)
          Component.onDestruction: {
            if (hovered)
              root.setAutohideEdgeHovered(false)
          }
        }
      }
    }
  ...
  component BarPanel: PanelWindow {
    id: barWindow
    // Autohide slide + fade
    property bool autohideParked: false

    readonly property string animationMode:
      root.autohideService ? root.autohideService.animationMode : "Slide + Fade"
    readonly property bool slideEnabled: animationMode !== "Fade"
    readonly property bool fadeEnabled: animationMode !== "Slide"

    readonly property real slideDistance:
      slideEnabled
        ? root.barSize * (
            root.autohideService
              ? root.autohideService.slideDistancePercent / 100
              : 1
          )
        : 0

    readonly property real hiddenOpacity: fadeEnabled ? 0 : 1
    readonly property bool customCurveEnabled:
      root.autohideService && root.autohideService.customCurveEnabled
    readonly property var customBezierCurve: root.autohideService
      ? [
          root.autohideService.curveX1,
          root.autohideService.curveY1,
          root.autohideService.curveX2,
          root.autohideService.curveY2,
          1, 1
        ]
      : [0.25, 0.1, 0.25, 1, 1, 1]

    readonly property int showEasingType:
      root.autohideService && root.autohideService.easing === "Quad" ? Easing.OutQuad :
      root.autohideService && root.autohideService.easing === "Quart" ? Easing.OutQuart :
      root.autohideService && root.autohideService.easing === "Quint" ? Easing.OutQuint :
      root.autohideService && root.autohideService.easing === "Sine" ? Easing.OutSine :
      Easing.OutCubic

    readonly property int hideEasingType:
      root.autohideService && root.autohideService.easing === "Quad" ? Easing.InQuad :
      root.autohideService && root.autohideService.easing === "Quart" ? Easing.InQuart :
      root.autohideService && root.autohideService.easing === "Quint" ? Easing.InQuint :
      root.autohideService && root.autohideService.easing === "Sine" ? Easing.InSine :
      Easing.InCubic

    readonly property real hiddenX:
      root.position === "left" ? -barWindow.slideDistance :
      root.position === "right" ? barWindow.slideDistance : 0

    readonly property real hiddenY:
      root.position === "top" ? -barWindow.slideDistance :
      root.position === "bottom" ? barWindow.slideDistance : 0

    Component.onCompleted: {
      autohideParked = root.barHidden

      if (root.barHidden) {
        barSlide.x = hiddenX
        barSlide.y = hiddenY
        barContent.opacity = barWindow.hiddenOpacity
      }
    }
    ...
    ParallelAnimation {
      ...
      NumberAnimation {
        ...
        property: "x"
        to: 0
        duration: root.autohideService
          ? root.autohideService.showSlideDuration
          : 500
        easing.type: barWindow.customCurveEnabled ? Easing.BezierSpline : barWindow.showEasingType
        easing.bezierCurve: barWindow.customBezierCurve
      }
      ...
      NumberAnimation {
        ...
        to: 1
        duration: root.autohideService
          ? root.autohideService.showFadeDuration
          : 400
        easing.type: barWindow.customCurveEnabled ? Easing.BezierSpline : barWindow.showEasingType
        easing.bezierCurve: barWindow.customBezierCurve
      }
    ...
    ParallelAnimation {
      ...
      NumberAnimation {
        target: barSlide
        property: "x"
        to: barWindow.hiddenX
        duration: root.autohideService
          ? root.autohideService.hideSlideDuration
          : 400
        easing.type: barWindow.customCurveEnabled ? Easing.BezierSpline : barWindow.hideEasingType
        easing.bezierCurve: barWindow.customBezierCurve
      }
      ...
      NumberAnimation {
        target: barContent
        property: "opacity"
        to: barWindow.hiddenOpacity
        duration: root.autohideService
          ? root.autohideService.hideFadeDuration
          : 320
        easing.type: barWindow.customCurveEnabled ? Easing.BezierSpline : barWindow.hideEasingType
        easing.bezierCurve: barWindow.customBezierCurve
      }
    ...
    Connections {
      ...
      function onBarHiddenChanged() {
        if (root.barHidden) {
          barWindow.autohideParked = false

          barShowAnimation.stop()
          barHideAnimation.restart()
        } else {
          barWindow.autohideParked = false

          barHideAnimation.stop()
          barShowAnimation.restart()
        }
      }
    }
